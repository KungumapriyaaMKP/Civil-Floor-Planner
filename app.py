...
# --------------------------------------------------
# Render pseudo-3D (ONLY after confirm)
# --------------------------------------------------
def render_3d(rooms, placed):
    ...
    d.text((20, 20), "3D View (Finalized Layout)", fill="black", font=font)
...
def move_room(plot, room_text, room_key, direction, state):
    if "placed" not in state or state.get("final"):
